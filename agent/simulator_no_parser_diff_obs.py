...
class Simulator(object):
    ...
    def __init__(self, 
        ...
        pomdp_file_added=None,
        policy_file_added=None,
        trials_num=1,
        num_task=1, 
        num_patient=1,
        num_recipient=1,
        ...
        self.pomdp_file_added=pomdp_file_added
        self.policy_file_added=policy_file_added
    ...
    def read_model_plus(self):
        ...
        model = pomdp_parser.Pomdp(filename=self.pomdp_file_added, parsing_print_flag=False)
        self.trans_mat_added = model.trans_mat
        self.obs_mat_added = model.obs_mat
        self.policy_added = policy_parser.Policy(len(self.states_plus), len(self.actions_plus), 
            filename=self.policy_file_added)
...
##########################################################
    ...
    def add_new(self, raw_str):
        ...
        # print self.observations
        self.trans_mat = self.trans_mat_added
        self.obs_mat = self.obs_mat_added
        self.reward_mat = self.reward_mat_plus
        
        self.policy = self.policy_added
...
def run_one():
    ...
    s = Simulator(uniform_init_belief = True, 
        ...
        pomdp_file =  name +'_new.pomdp',
            ...
            pomdp_file_added=list(name)[0]+str(int(list(name)[1])+1)+str(int(list(name)[2])+1)+'_new.pomdp',
            policy_file_added=list(name)[0]+str(int(list(name)[1])+1)+str(int(list(name)[2])+1)+'_new.policy',
        trials_num = num,
        num_task = int(name[0]), 
