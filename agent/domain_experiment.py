...
def hypo3(df1,df2,df3,filelist,num):
	...
	plt.subplot(131)
	plt.errorbar([17,26,37],df1.loc[filelist[0]:filelist[-1],'QA Cost'], yerr=ourAgentQAList, color='y', capsize=5, marker='*',linestyle='-',label='Dual-track POMDP Manager')
...
def stdDev(l):
    Std17 = numpy.std(l[0:13:3])
    Std26 = numpy.std(l[1:14:3])
    Std37 = numpy.std(l[2::3])
...
def main():
	...
	filelist=['144','155','166']  
        batchNum = 5                   #list of pomdp files
	...
	# just use for sth in somelist, not for sth in range(len(ssomelist))
		...
		#Put i or name or whatever the name of the iterator is, below in df.at[i, e.g. "Overall Cost"]
		    ...
		    df1.at[iterator,'QA Cost']= a
                    ourAgentQAList.append(a)
		    df1.at[iterator,'Overall Success']= b
                    ourAgentSuccess.append(b)
		    df1.at[iterator,'Dialog Reward']= c
                    ourAgentReward.append(c)
	...
	print (df3)
        ...
        ourQA = stdDev(ourAgentQAList)
        ourSuc = stdDev(ourAgentSuccess)
        ourRew = stdDev(ourAgentReward)
        ourF1 = stdDev(ourAgentF1)
        ourAcc = stdDev(ourAgentAcc)

        baseQA = stdDev(baseBeliefQAList)
        baseSuc = stdDev(baseBeliefSuccess)
        baseRew = stdDev(baseBeliefReward)
        baseF1 = stdDev(baseBeliefF1)
        baseAcc = stdDev(baseBeliefAcc)

        baseQA_2 = stdDev(baseEFQAList)
        baseSuc_2 = stdDev(baseEFSuccess)
        baseRew_2 = stdDev(baseEFReward)
        baseF1_2 = stdDev(baseEFF1)
        baseAcc_2 = stdDev(baseEFAcc)
