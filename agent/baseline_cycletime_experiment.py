...
def plotgenerate(df,belieflist,num):
   ...
   '''
    ...
    #belieflist = np.array(belieflist)
...
def main():
    ...
    cycletimeList=[2,3,4,5,6,7,8]
    #filelist = ['133', '144']
    df=pd.DataFrame() 
    # just use for sth in somelist, not for sth in range(len(ssomelist))
    for iterator in cycletimeList:
        name = '155'  # or name = iterator

        s = Baseline(uniform_init_belief = True, 
            auto_state = True, 
        ...
        a,b,c,p,r,f=s.run_numbers_of_trials(iterator)
    ...
    print df
    df.to_csv("Plots_data/all"+str(num)+"_trials_N_experiment_baseline_model_133.csv", encoding='utf-8', index=True)
    plotgenerate(df,cycletimeList,num)
