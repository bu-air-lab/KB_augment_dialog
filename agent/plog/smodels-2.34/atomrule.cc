...
ConstraintRule::print_internal ()
{
  ...
  int true_literals = 0;
  Atom **a;
  for (a = nend; a != pbody; a++)
    if ((*a)->Bneg)
      true_literals++;
  for (a = pend; a != end; a++)
    if ((*a)->Bpos)
      true_literals++;
  cout << ' ' << (true_literals >= atleast ? 0 : atleast-true_literals);
  for (a = nbody; a != nend; a++)
    {
      cout << ' ' << (*a)->posScore;
      (*a)->negScore = 1;  // Mark as needed in compute statement
...
# endif
  ...
  Atom **a;
  for (a = bend; a != end; a++)
    if ((aux[a-body].positive == true && (*a)->Bpos) ||
	(aux[a-body].positive == false && (*a)->Bneg))
      weight += aux[a-body].weight;
  cout << ' ' << (weight >= atleast ? WEIGHT_MIN : atleast-weight) << ' '
       << bend-body;
  long neg = 0;
  for (a = body; a != bend; a++)
